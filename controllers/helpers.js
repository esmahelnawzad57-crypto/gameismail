...
function get3Words(roomID) {
    const { probability: p = 0 } = games[roomID] || {};
    const customWords = (games[roomID] && games[roomID].customWords) || [];
    if (customWords.length < 3) return chance.pickset(words, 3);
    const pickedWords = new Set();
    while (pickedWords.size !== 3) {
        const wordSet = chance.weighted([words, customWords], [1 - p, p]);
