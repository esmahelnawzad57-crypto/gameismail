    ...
    onMessage(data) {
        ...
        if (distance === 0 && currentWord !== '') {
            ...
            if (games[socket.roomID].drawer !== socket.id && !socket.hasGuessed) {
                ...
                socket.emit('correctGuess', { message: 'ڕاستت زانی!', id: socket.id });
                socket.broadcast.emit('correctGuess', { message: `${socket.player.name} وشەکەی زانی!`, id: socket.id });
            ...
            if (games[socket.roomID].drawer !== socket.id && !socket.hasGuessed) socket.emit('closeGuess', { message: 'زۆر نزیک بوویتەوە!' });
        } else {
            io.in(socket.roomID).emit('message', { ...data, name: socket.player.name });
        }
    }
